...
# deploys the declarative Grafana Admin Platform Helm chart into your cluster.

OPERATOR_VERSION="${GRAFANA_OPERATOR_VERSION:-5.24.0}"
OPERATOR_NAMESPACE="${GRAFANA_OPERATOR_NAMESPACE:-grafana-operator}"
RESOURCE_NAMESPACE="${GRAFANA_RESOURCE_NAMESPACE:-grafana-operator-configs}"
RELEASE_NAME="${GRAFANA_OPERATOR_RELEASE_NAME:-grafana-operator}"
CHART_RELEASE_NAME="${GRAFANA_ADMIN_RELEASE_NAME:-grafana-admin-platform}"
GRAFANA_URL="${GRAFANA_URL:-https://cosmicsatish.grafana.net}"
GRAFANA_SECRET_NAME="${GRAFANA_SECRET_NAME:-grafanacloud-credentials}"
GRAFANA_SECRET_KEY="${GRAFANA_SECRET_KEY:-GRAFANA_API_KEY}"
...
# Manage Admin Token Secret (grafanacloud-credentials / GRAFANA_API_KEY)
if ! kubectl -n "$RESOURCE_NAMESPACE" get secret "$GRAFANA_SECRET_NAME" >/dev/null 2>&1; then
  ...
  if [[ -n "$GRAFANA_TOKEN" ]]; then
    kubectl -n "$RESOURCE_NAMESPACE" create secret generic "$GRAFANA_SECRET_NAME" \
      --from-literal="$GRAFANA_SECRET_KEY"="$GRAFANA_TOKEN" \
...
Create it using your approved secret manager or rerun interactively so the token can be entered securely:
  kubectl create secret generic $GRAFANA_SECRET_NAME -n $RESOURCE_NAMESPACE --from-literal=$GRAFANA_SECRET_KEY="<YOUR_TOKEN>"
MSG
  exit 1
fi
